   ...
   var TBTimer = function(config){
      ...
      this.updateDisplay = function(ms){
         ...
         this.displayEl.innerHTML = 
            pad(tm.minutes) + ':' + 
            pad(tm.seconds);// + '.' +
            //Math.floor(tm.millis);
			
		 this.displayEl2.innerHTML = Math.floor(tm.millis);
      ...
      this._doRound = function(rn){
         ...
         if(this.rounds[rn]){
            var __tick = function(){
			   ...
			   //$('progress').val((self.rounds[rn].total - dtm)/self.rounds[rn].total);
			   $('#filled_bar').width((self.rounds[rn].total - dtm)*100/self.rounds[rn].total + '%');
            ...
            if(current.round.type == 'rest'){
			   $('.timer-display').css('color', '#00FF00');
			   $('div.div2').css('color', '#00FF00');
               this.notify('Rest!');
               if(rn != 0){
               ...
               this.notify('Work!');
			   $('.timer-display').css('color', '#E8F1DA'); 
			   $('div.div2').css('color', '#E8F1DA');
      ...
      this.notifyForRound = function(rn, type){
         $('.round-number').html(rn + "/" + $('.input-rounds').val());
